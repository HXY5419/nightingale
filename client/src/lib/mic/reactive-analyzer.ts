...
export interface MicReactiveEvent {
  volume: number;
  low: number;
  mid: number;
  high: number;
...
export class ReactiveAnalyzer {
  ...
  analyze(samples: Float32Array, sampleRate: number): MicReactiveEvent {
    ...
    let sumSq = 0;
    for (let i = 0; i < REACTIVE_FFT_SIZE; i++) {
      const s = samples[i];
      sumSq += s * s;
      this.real[i] = s * this.window[i];
      this.imag[i] = 0;
    }
    const rawRms = Math.sqrt(sumSq / REACTIVE_FFT_SIZE);

    fftInPlace(this.real, this.imag, REACTIVE_FFT_SIZE);

    const binCount = REACTIVE_FFT_SIZE >> 1;
    ...
    for (let i = 0; i < binCount; i++) {
      ...
      const mag = Math.sqrt(re * re + im * im) * magNorm;
      const freq = i * binHz;
    ...
    const centroidRaw = centroidDen > 1e-6 ? clamp01(centroidNum / centroidDen / nyquist) : 0;

    const bandNorm = (sum: number, count: number): number => {
      if (count === 0) return 0;
      return clamp01((sum / count) * BAND_NORM_GAIN);
    };
    const lowRaw = bandNorm(lowSum, lowCount);
    const midRaw = bandNorm(midSum, midCount);
    const highRaw = bandNorm(highSum, highCount);
    ...
    if (peakMag > 0 && rawRms > this.rmsGate) {
