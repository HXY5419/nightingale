import { onSetupError, onSetupProgress, skipVendorSetup, triggerSetup } from "@/bridge/setup";
...
import { selectFolderPath, selectFilePath } from "@/bridge/source";
...
interface ExtendedSetupProgress extends Omit<SetupProgress, "step"> {
  step: SetupStep | "init" | "error" | "changedatafolder" | "skipsource";
}
...
const InitialStep = ({ toNextStep, toSkipSource }: InitialStepProps) => {
  return (
    <>
      <AlertDialogHeader>
      ...
      <AlertDialogFooter className="flex-row-reverse justify-between sm:justify-between">
        ...
        <AlertDialogAction
          onClick={toSkipSource}
          className="bg-muted text-muted-foreground hover:bg-muted/80 hover:text-muted-foreground"
        >
          Skip Download
        </AlertDialogAction>
      </AlertDialogFooter>
...
const FilePicker = ({
  ...
  <div className="grid w-full justify-self-stretch grid-cols-[minmax(0,1fr)_auto] gap-2">
    <Input
      ...
      placeholder="Select or type the file path..."
    ...
    <Button
      ...
      className="w-fit whitespace-nowrap"
      onClick={async () => {
        const file = await selectFilePath();
    ...
    >
      {value ? "Change File" : (buttonLabel ?? "Choose File")}
    </Button>
  </div>
);
...
const SkipSourceStep = ({
  ...
  <>
    <AlertDialogHeader className="w-full place-items-stretch text-left">
      <AlertDialogTitle>Skip Download</AlertDialogTitle>
      <AlertDialogDescription className="mb-2">
        Choose the locations of your existing <code>ffmpeg</code> and <code>python 3.10</code>{" "}
        executables. Nightingale will use them directly instead of downloading new copies.
      </AlertDialogDescription>
      <div className="mt-2 flex w-full flex-col gap-4">
        <div className="flex w-full flex-col gap-1">
          <div className="text-sm font-medium">ffmpeg path</div>
          <div className="text-xs text-muted-foreground mb-1">
            Path to your existing <code>ffmpeg</code> executable (optional)
          </div>
          <FilePicker
            value={ffmpegPath}
            onChange={(file) => setFfmpegPath(file ?? "")}
            buttonLabel="Choose ffmpeg"
          />
        </div>
        <div className="flex w-full flex-col gap-1">
          <div className="text-sm font-medium">Python 3.10 path</div>
          <div className="text-xs text-muted-foreground mb-1">
            Path to your existing <code>python3.10</code> executable (optional)
          </div>
          <FilePicker
            value={pythonPath}
            onChange={(file) => setPythonPath(file ?? "")}
            buttonLabel="Choose Python"
          />
        </div>
      </div>
    </AlertDialogHeader>
    <AlertDialogFooter className="flex-row-reverse justify-between sm:justify-between">
      <div className="flex gap-2">
        <AlertDialogAction onClick={onSkip}>Skip &amp; Continue</AlertDialogAction>
        {EXIT_SUPPORTED && <AlertDialogCancel onClick={() => exit()}>Exit</AlertDialogCancel>}
      </div>
      <AlertDialogAction onClick={onBack} className="bg-muted text-muted-foreground hover:bg-muted/80 hover:text-muted-foreground">
        Back
      </AlertDialogAction>
    </AlertDialogFooter>
  </>
...
export const Setup = () => {
  ...
  const startSkipSetup = useCallback(async () => {
    setSetupProgress({ step: "clearvendor", percent: 10, action: "Copying user-provided components..." });
    // Fire-and-forget: progress and error states are handled by the event listeners
    // (onSetupProgress / onSetupError) wired up in the useEffect below.
    await skipVendorSetup(ffmpegPath || undefined, pythonPath || undefined);
      ...
      case "init":
        return () => (
          <InitialStep
            ...
            toSkipSource={() => setSetupProgress({ ...setupProgress, step: "skipsource" })}
          />
        );
      ...
      case "skipsource":
        return () => (
          <SkipSourceStep
            ...
            onSkip={startSkipSetup}
            onBack={() => setSetupProgress({ ...setupProgress, step: "init" })}
          />
        );
      case "clearvendor":
      case "ffmpeg":
