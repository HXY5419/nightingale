use app_core::{is_ready as is_vendor_ready, run_vendor_setup, skip_vendor_setup, CachePaths, SetupFolders};
...
pub fn skip_setup(app: AppHandle, ffmpeg_path: Option<String>, python_path: Option<String>) {
    std::thread::spawn(move || {
        ...
        if let Err(e) = skip_vendor_setup(user_ffmpeg, user_python) {
            let _ = app.emit("setup-error", e);
            return;
        }
        let _ = app.emit(
