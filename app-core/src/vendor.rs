...
/// Skip the automatic vendor download and use user-provided component paths instead.
/// Copies the user's ffmpeg to the vendor dir, then creates the venv directly from the
/// user's Python installation (without copying it) and installs all required packages.
pub fn skip_vendor_setup(
    user_ffmpeg: Option<PathBuf>,
    user_python: Option<PathBuf>,
) -> Result<(), String> {
    let _ = std::fs::create_dir_all(vendor_dir());

    if let Some(ffmpeg_src) = &user_ffmpeg {
        ...
        let dest = ffmpeg_path();
        std::fs::copy(ffmpeg_src, &dest).map_err(|e| {
            format!(
                "Failed to copy ffmpeg from {} to {}: {e}",
                ffmpeg_src.display(),
                dest.display()
            )
        })?;
    ...
    if let Some(python_src) = &user_python {
        if !python_src.is_file() {
            return Err(format!(
                "Python not found at: {}",
                python_src.display()
            ));
        ...
        // uv is a small single binary; download it so we can manage the venv and packages.
        step_download_uv()?;

        // Create the venv directly from the user's Python installation.
        // We cannot use step_create_venv() here because it calls find_installed_python(),
        // which expects python to be pre-installed inside vendor/python/ — copying a bare
        // python.exe there would lose its stdlib and DLLs, breaking the venv.
        let venv_dir = vendor_dir().join("venv");
        if !python_path().is_file() {
            let output = silent_command(uv_path())
                .args(["venv", "--python"])
